...
class DebugFrameTiming : ISystem() {
    ...
    override fun render(entities: List<Entity>) {
        ...
        val renderer = YapGame.getInstance().fontRenderer
        val screenWidth = 4 * aspectRatio
        val barWidth = screenWidth / NUMBER_OF_TIMINGS / 2.0F
        ...
        for (timingWithIndex in timings.withIndex()) {
            ...
            renderTiming(renderer, aspectRatio, barWidth, timingWithIndex.index, timingWithIndex.value)
        }
    ...
    private fun renderTiming(renderer: FontRenderer, aspectRatio: Float, width: Float, index: Int, timing: Duration) {
        val barHeight = timing.toMillis() / TARGET_FRAME_TIME / 3.0F
        val x = -aspectRatio + (index + 0.5F) * width
        val y = -1.0F + barHeight / 2.0F
        val transformation = Matrix4f()
                .translate(x, y, 0.0F)
                .scale(width, barHeight, 0.0F)

        val color = if (index == currentIndex - 1) {
            Vector4f(1.0F, 1.0F, 0.0F, 1.0F)
        } else {
            val red = Vector4f(1.0F, 0.0F, 0.0F, 1.0F)
            val green = Vector4f(0.0F, 1.0F, 0.0F, 1.0F)
            val colorDir = Vector4f(red).sub(green).mul(barHeight - TARGET_FRAME_TIME / 100.0F)
